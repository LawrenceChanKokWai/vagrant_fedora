...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    # moving the 2 files to /tmp/ before installing httpd -> Clean and update yum
    ...
    # Installing, enable, start httpd
    yum install httpd wget unzip -y
    sudo systemctl enable httpd
    sudo systemctl start httpd

    # Installing the webpage to /tmp/ directory
    # copy it to /var/www/html/ folder
    # systemctl restart httpd
    cd /tmp/
    wget https://templatemo.com/tm-zip-files-2020/templatemo_506_tinker.zip
    unzip -o templatemo_506_tinker.zip
